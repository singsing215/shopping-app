...
var ChartBaseValueMapper = /** @class */ (function () {
            ...
            if (collectionData.action && collectionData.action.toLowerCase() === "add") {
                if (chart.androidView && chart.series) {
                    for (var i = 0; i < collectionData.addedCount; i++) {
                        var currentSeries = chart.series.getItem(collectionData.index + i);
                        ...
                        this.setAxisPaletteValues(this.resolveHorizontalAxisForSeries(currentSeries), "HorizontalAxis", chart);
                        this.setAxisPaletteValues(this.resolveVerticalAxisForSeries(currentSeries), "VerticalAxis", chart);
                    }
                }
                return;
            }
            if (collectionData.action && collectionData.action.toLowerCase() === "splice") {
                if (collectionData.removed instanceof observable_array_1.ObservableArray) {
                    for (var serieIndex = 0; serieIndex < collectionData.removed.length; serieIndex++) {
                        var currentSeries = collectionData.removed.getItem(serieIndex);
                    ...
                    for (var serieIndex = 0; serieIndex < collectionData.removed.length; serieIndex++) {
                        var currentSeries = collectionData.removed[serieIndex];
                        chart._removeView(currentSeries);
                        if (chart.androidView.getSeries().indexOf(currentSeries.android) !== -1) {
                            chart.androidView.getSeries().remove(currentSeries.android);
                        }
        ...
        if (oldValue) {
            if (chart.androidView) {
                for (var serieIndex = 0; serieIndex < oldValue.length; serieIndex++) {
                    var currentSeries = oldValue.getItem(serieIndex);
                    chart._removeView(currentSeries);
                    if (chart.androidView.getSeries().indexOf(currentSeries.android) !== -1) {
                        chart.androidView.getSeries().remove(currentSeries.android);
                    }
    ...
    ChartBaseValueMapper.prototype.loadSeries = function (chart) {
        if (chart.androidView && chart.series) {
            for (var i = 0; i < chart.series.length; i++) {
                var series = chart.series.getItem(i);
...
var CartesianAxisValueMapper = /** @class */ (function () {
    ...
    CartesianAxisValueMapper.prototype.onLabelMarginChanged = function (oldValue, newValue, axis) {
        if (!isNaN(newValue)) {
            axis.android.setLabelMargin(newValue * utilsModule.layout.getDisplayDensity());
        }
    };
    ...
    CartesianAxisValueMapper.prototype.onFontInternalChanged = function (oldValue, newValue, axis) {
        if (newValue) {
            var androidFont = newValue.getAndroidTypeface();
            axis.android.setLabelFont(androidFont);
            var androidSize = newValue.fontSize * utilsModule.layout.getDisplayDensity();
...
var ChartSeriesValueMapper = /** @class */ (function () {
    ...
    ChartSeriesValueMapper.prototype.onLabelPaddingChanged = function (newValue, propertyName, series) {
        var paddingLeftFromStyle = series.label.style.paddingLeft;
        var paddingLeft = (!propertyName || propertyName === "paddingLeft") && !isNaN(+paddingLeftFromStyle) ?
            +paddingLeftFromStyle : series.android.getLabelPaddingLeft();
        var paddingTopFromStyle = series.label.style.paddingTop;
        var paddingTop = (!propertyName || propertyName === "paddingTop") && !isNaN(+paddingTopFromStyle) ?
            +paddingTopFromStyle : series.android.getLabelPaddingTop();
        var paddingBottomFromStyle = series.label.style.paddingBottom;
        var paddingBottom = (!propertyName || propertyName === "paddingBottom") && !isNaN(+paddingBottomFromStyle) ?
            +paddingBottomFromStyle : series.android.getLabelPaddingBottom();
        var paddingRightFromStyle = series.label.style.paddingRight;
        var paddingRight = (!propertyName || propertyName === "paddingRight") && !isNaN(+paddingRightFromStyle) ?
    ...
    ChartSeriesValueMapper.prototype.applyFontInternal = function (fontInternal, series) {
        if (fontInternal) {
            var androidFont = fontInternal.getAndroidTypeface();
            if (androidFont) {
                series.android.setLabelFont(androidFont);
            }
            if (!isNaN(fontInternal.fontSize)) {
                var androidSize = fontInternal.fontSize * utilsModule.layout.getDisplayDensity();
...
var CartesianSeriesValueMapper = /** @class */ (function (_super) {
    ...
    CartesianSeriesValueMapper.prototype.onStrokeWidthChanged = function (oldValue, newValue, series) {
        var devicePixels = utilsModule.layout.toDevicePixels(newValue);
...
var PieSeriesValueMapper = /** @class */ (function (_super) {
    ...
    PieSeriesValueMapper.prototype.onStartAngleChanged = function (oldValue, newValue, series) {
        if (!isNaN(+newValue)) {
            var sweepAngle = series.endAngle ? series.endAngle - newValue : 360;
    ...
    PieSeriesValueMapper.prototype.onEndAngleChanged = function (oldValue, newValue, series) {
        if (!isNaN(+newValue)) {
            var startAngle = series.startAngle ? series.startAngle : 0;
            var sweepAngle = newValue - startAngle;
    ...
    PieSeriesValueMapper.prototype.applyColors = function (series) {
        var fillColors = series.fillColors;
        var fillColorsLength = fillColors ? fillColors.length : 0;
        var strokeColors = series.strokeColors;
        var strokeColorsLength = strokeColors ? strokeColors.length : 0;
        var length = Math.max(fillColorsLength, strokeColorsLength);
        var nativeSliceStyles = new java.util.ArrayList();
        for (var i = 0; i < length; i++) {
            var style = new com.telerik.widget.chart.visualization.pieChart.SliceStyle();
...
var LineSeriesValueMapper = /** @class */ (function (_super) {
    ...
    LineSeriesValueMapper.prototype.onFillColorChanged = function (oldValue, newValue, series) {
...
var AreaSeriesValueMapper = /** @class */ (function (_super) {
    ...
    AreaSeriesValueMapper.prototype.onFillColorChanged = function (oldValue, newValue, series) {
        series.android.setFillColor(newValue.android);
    };
...
var OhlcSeriesValueMapper = /** @class */ (function (_super) {
    ...
    OhlcSeriesValueMapper.prototype.onFillColorChanged = function (oldValue, newValue, series) {
        console.log('WARNING: Property fillColor is not applicable for series of type: ' + series.constructor.name);
    };
...
var CandlestickSeriesValueMapper = /** @class */ (function (_super) {
    ...
    CandlestickSeriesValueMapper.prototype.onFillColorChanged = function (oldValue, newValue, series) {
        series.android.setFillColor(newValue.android);
    };
